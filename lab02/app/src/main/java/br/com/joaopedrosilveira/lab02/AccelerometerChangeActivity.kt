...
class AccelerometerChangeActivity: ComponentActivity() {
            ...
            Lab02Theme {
                Surface(modifier = Modifier.fillMaxSize(), color = MaterialTheme.colors.background)
                {
                    Column(
                        ...
                        Text("Accelerometer changed abruptly")
                        Spacer(modifier = Modifier.height(8.dp))
                        GoBackButton(onClick = {
                            goBack(this@AccelerometerChangeActivity)
                        })
                    }
                }
            }
...
private fun goBack(context: Context) {
    val intent = Intent(context, MainActivity::class.java).apply {
        putExtra(Extra.SCREEN.name, Screen.ACCELEROMETER.name)
    }
